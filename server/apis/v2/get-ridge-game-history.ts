...
export default api({
  name: "GetRidgeGameHistory",
  description: "Fetches a learner's Ridge Runner game sessions for analytics",
  ...
  async run(ctx, { viewerId }) {
    const sessions = await ctx.integrations.apps_db.query(
      `SELECT id AS session_id, is_replay, net_xp, badge_key,
      ...
      SessionSchema,
      [viewerId],
      { label: "Fetch ridge game history" }
    );
    ...
    const BADGE_RANK: Record<string, number> = {
      ...
      ridge_rookie: 2,
      ridge_trail_judge: 3,
      ridge_roe_enforcer: 4,
      ridge_summit_authority: 5,
    };
