...
/** Badge tiers determined by net XP change */
const RIDGE_BADGES = [
  { min: -20, max: -1,  badgeId: "ridge_whipper",          name: "Whipper",          emoji: "🪢" },
  { min: 0,   max: 9,   badgeId: "ridge_rookie",           name: "Ridge Rookie",     emoji: "🪨" },
  { min: 10,  max: 19,  badgeId: "ridge_trail_judge",      name: "Trail Judge",      emoji: "⛏️" },
  { min: 20,  max: 26,  badgeId: "ridge_roe_enforcer",     name: "ROE Enforcer",     emoji: "🏔️" },
  { min: 27,  max: 30,  badgeId: "ridge_summit_authority",  name: "Summit Authority", emoji: "🌄" },
];

function getBadgeTier(netXp: number) {
  return RIDGE_BADGES.find((b) => netXp >= b.min && netXp <= b.max) ?? RIDGE_BADGES[0];
}

export default api({
  name: "CompleteRidgeGame",
  description: "Finalizes a Ridge game session, awards badge and XP",
  ...
  async run(ctx, { sessionId, viewerId, clipId, isReplay }) {
    ...
    const responses = await ctx.integrations.apps_db.query(
      `SELECT scenario_id, is_correct, xp_change, crux_level
       FROM cliptracker_v2_ridge_responses
       WHERE session_id = $1
       ORDER BY answered_at`,
      ResponseSchema,
      [sessionId],
      { label: "Get Ridge session responses" }
    );
    ...
    const cruxAccuracy = highConfidence.length > 0
      ...
      : -1; // -1 = no high-confidence wagers made

    // Section breakdown — join with scenarios to get section names
    ...
    const sectionBreakdown = await ctx.integrations.apps_db.query(
      `SELECT s.section,
              ...
              COUNT(*)::int as total
       FROM cliptracker_v2_ridge_responses r
       JOIN cliptracker_v2_ridge_scenarios s ON s.id = r.scenario_id
       WHERE r.session_id = $1
       GROUP BY s.section
       ORDER BY s.section`,
      SectionSchema,
      [sessionId],
      { label: "Get Ridge section breakdown" }
    );
    ...
    await ctx.integrations.apps_db.execute(
      `UPDATE cliptracker_v2_ridge_sessions
       SET net_xp = $1, badge_key = $2, completed_at = NOW()
       WHERE id = $3`,
      [netXp, badge.badgeId, sessionId],
      { label: "Finalize Ridge session" }
    );
    ...
    if (!isReplay) {
      ...
      if (!isAdmin) {
        ...
        for (const r of responses) {
          if (r.xp_change !== 0) {
            await ctx.integrations.apps_db.execute(
              ...
              [viewerId, clipId, "ridge_game", `ridge_${sessionId}_${r.scenario_id}`, r.xp_change],
              { label: `Ridge XP: ${r.xp_change > 0 ? "+" : ""}${r.xp_change}` }
            );
          }
        }
        ...
        await ctx.integrations.apps_db.execute(
          ...
          { label: `Award Ridge badge: ${badge.name}` }
        );
      }
    }

    // Mark ROE clip as completed for pacing by inserting a completed session.
    // The pacing system counts clips with at least 1 completed session row.
    // Only insert if no completed session exists yet (avoids duplicates on replays).
    await ctx.integrations.apps_db.execute(
      ...
      [clipId, viewerId],
      { label: "Mark ROE clip completed for pacing" }
    );
