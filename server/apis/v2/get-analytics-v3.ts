...
const LearnerRow = z.object({
  ...
  trail_score_avg: z.string().nullable(),
...
export default api({
  ...
  output: z.object({
    ...
    learners: z.array(z.object({
      ...
      totalXp: z.number(),
      trailScoreAvg: z.number().nullable(),
  ...
  async run(ctx) {
    ...
    const learnerRows = await ctx.integrations.db.query(
      `SELECT
        ...
        ROUND(AVG(s.question_score) FILTER (WHERE s.completed = true AND s.is_recovery_attempt = false), 1)::text AS trail_score_avg,
    ...
    const learners = learnerRows.map(l => {
      ...
      return {
        ...
        totalXp: l.total_xp,
        trailScoreAvg: l.trail_score_avg ? parseFloat(l.trail_score_avg) : null,
